...
const ActiveCreditPanel = ({
  ...
  const handleFinalize = () => {
    if (!activeCredit) {
      console.error("No hay un crédito activo para finalizar.");
      return;
    }
    if (activeCreditItems.length === 0) {
      console.error("El crédito no puede estar vacío para finalizar.");
      return;
    }
    if (!selectedClientOption || !selectedClientOption.value) {
      console.error("Por favor, selecciona un cliente para finalizar el crédito.");
      return;
    }
              ...
              >
                ...
                <div className="px-4 py-6 sm:px-6">
                  ...
                  <div className="border-b pb-4">
                    <h3 className="text-md font-semibold text-gray-800 mb-2">Créditos Pendientes</h3>
                    {pendingCredits.length === 0 ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                        {pendingCredits.map(credit => (
                          <div
                            key={credit.id}
                            className={`p-2 border rounded-md text-sm cursor-pointer ${activeCredit && activeCredit.id === credit.id ? 'bg-yellow-100 border-yellow-500 font-medium' : 'bg-gray-50 hover:bg-gray-100'}`}
                            onClick={() => onSelectPendingCredit(credit.id)}
                          >
                            <p><strong>Número:</strong> {credit.numeroCredito}</p>
                            <p><strong>Cliente:</strong> {credit.clienteNombre}</p>
                            <p><strong>Total:</strong> S/. {parseFloat(credit.totalCredito || 0).toFixed(2)}</p>
                            <p className="text-xs text-gray-500">
                              Creado: {credit.fechaCreacion ? new Date(credit.fechaCreacion.toDate()).toLocaleString() : 'N/A'}
                            </p>
                            {activeCredit && activeCredit.id === credit.id && <p className="text-xs text-yellow-700 mt-1">Crédito activo</p>}
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  {activeCredit ? (
                    ...
                    <div className="mt-8 text-center text-gray-500">
                      ...
                      <p>Crea un nuevo crédito o selecciona uno de los pendientes.</p>
                    </div>
                  )}
                </div>
                ...
                <div className="border-t border-gray-200 px-4 py-6 sm:px-6">
                  ...
                  <div className="mt-6">
                    <button
                      ...
                      className="flex items-center justify-center rounded-md border border-transparent bg-yellow-600 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-yellow-700 w-full"
                      disabled={!activeCredit || activeCreditItems.length === 0}
                    >
                      Finalizar Crédito
                    </button>
                  </div>
